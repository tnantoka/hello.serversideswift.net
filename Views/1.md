...
最低限の方法は、[Serving content written in Markdown using Kitura - Swift@IBM](https://developer.ibm.com/swift/2016/12/12/serving-content-written-in-markdown-using-kitura/)にやり方が載っています。  
今回layoutなども実現したかったので、[IBM-Swift/Kitura-StencilTemplateEngine](https://github.com/IBM-Swift/Kitura-StencilTemplateEngine)と組み合わせました。
...
僕はだいぶ前にアカウントを作っていて、無料トライアルが終わってしまっていたので、本登録をしました。
...
今回は`serversideswift.net`ドメインを追加して、`www`を経路として追加しました。  
参考までに僕の設定画面をキャプチャしておきます。

![](/images/1/routes.png)
![](/images/1/domains.png)

あとはCNAMEを設定すれば、`https://www.serversideswift.net/`でもアクセスできるようになります。
